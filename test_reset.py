...
def min_slots_reset(tasks_data, k):
    """Version where interrupt RESETS ready_time (no max)"""
    ready_queue = []
    ...
    while total_tasks > 0:
        ...
        # Apply interrupts - RESET without max()
        for t in cooldown_list:
            if t.priority < current.priority:
                t.ready_time = time + k + 1  # NO max()
...
# Test
result = min_slots_reset([('A', 13, 2), ('B', 11, 2), ('C', 18, 1)], 8)
print(f"Reset without max: {result}")
