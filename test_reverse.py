...
def min_slots_reverse_interrupt(tasks_data, k):
    """Version where LOWER priority interrupts HIGHER (opposite rule)"""
    ready_queue = []
    ...
    while total_tasks > 0:
        ...
        # REVERSE: current interrupts HIGHER priority tasks
        for t in cooldown_list:
            if t.priority > current.priority:  # REVERSED
...
# Test
result = min_slots_reverse_interrupt([('A', 13, 2), ('B', 11, 2), ('C', 18, 1)], 8)
print(f"Reverse interrupt: {result}")
