...
def min_slots_skip_idle(tasks_data, k):
    """Version that skips idle time"""
    ready_queue = []
    ...
    while total_tasks > 0:
        ...
        if not ready_queue:
            # Jump to next ready time
            if cooldown_list:
                min_ready = min(t.ready_time for t in cooldown_list)
                time = min_ready - 1  # Will be incremented to min_ready
            continue
...
# Test
result = min_slots_skip_idle([('A', 13, 2), ('B', 11, 2), ('C', 18, 1)], 8)
print(f"Skip idle: {result}")
